...
order: 1
# ── CHIP CONFIG ───────────────────────────────────────────────────────────
# Add/remove chips for THIS presentation here. Each chip shows on the
# "Papers & Presentations" list and links to a heading (anchor) on this page.
#   label : PDF | VIDEO | CODE   (the text shown in the chip)
#   anchor: the heading id it jumps to on this page
#           "## Paper" -> paper, "## Presentation video" -> presentation-video,
#           "## Code"  -> code   (kramdown lowercases + hyphenates headings)
# Delete a chip to hide it. Leave the list empty (chips: []) for no chips.
chips:
  - label: PDF
    anchor: paper
  - label: VIDEO
